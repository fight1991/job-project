<template>
  <section class="batch-sync-dialog">
    <el-dialog
      ...
      @open="initUpload"
      @close="$emit('update:uploadVisible', false)"
      width="1050px">
              ...
              <el-form :model="singleTableList[0]" label-width="95px" size="mini">
                <el-col :span="8">
                  <el-form-item label="申报截止时间">
                    <el-date-picker v-model="singleTableList[0].demandDate" size="mini" type="datetime" placeholder="" value-format="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm" default-time="23:59:59" style="width:100%;"></el-date-picker>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
      ...
      <el-row v-else>
        <el-row class="dec-query-table">
          <el-table class='sys-table-table dec-table'
            :data="ocrInfo.files" border highlight-current-row size="mini">
        ...
        <el-row :gutter="20" class="mg-t-20">
          <el-form :model="ocrInfo" size="mini">
            <el-col :span="7">
              <el-form-item label="申报截止时间:" label-width="97px">
                <div class="break-word">{{ocrInfo.demandDate}}</div>
              </el-form-item>
            </el-col>
            <el-col :span="9">
              <el-form-item label="委托客户:" label-width="69px">
                <div class="break-word">{{ocrInfo.entrustCompanyName}}</div>
              </el-form-item>
            </el-col>
            <el-col :span="8">
      ...
      <el-row class="mg-t-20" v-if="operationType == 'add'">
        <el-col :span="24" align="center">
          ...
          <el-button size="mini" @click="$emit('update:uploadVisible', false)">取消</el-button>
        </el-col>
      </el-row>
    </el-dialog>
  </section>
...
export default {
  ...
  methods: {
    initUpload () {
      if (this.operationType === 'add') {
        this.singleTableList = [{
          ...
          entrustCompanyName: '',
          note: '',
          taskTime: util.dateFormat(new Date(), 'yyyy-MM-dd hh:mm'),
          ocrNo: '',
          taskType: '0'
        }]
      ...
      if (file.type && !allow.includes(file.type)) {
        ...
        if (type === 'single') {
          this.singleTableList[0].files.push({
            ...
            fileUrl: '',
            progress: 0,
            uploadTime: util.dateFormat(new Date(), 'yyyy-MM-dd hh:mm'),
            status: 'success',
            uid: file.uid,
            size: file.size
          })
          ...
          this.multipleTableList.push({
            files: [{fileName: file.name, fileUrl: '', uploadTime: util.dateFormat(new Date(), 'yyyy-MM-dd hh:mm')}],
            demandDate: '',
            entrustCompanyName: '',
            note: '',
            taskTime: util.dateFormat(new Date(), 'yyyy-MM-dd hh:mm'),
    ...
    confirmForm () {
      ...
      this.$post({
        url: 'API@/dec-common/dec/orc/uploadDecDocument',
        data: data,
        isLoad: false,
        success: (res) => {
          this.$message({
