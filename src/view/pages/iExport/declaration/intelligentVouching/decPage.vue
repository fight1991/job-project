<template>
  ...
  <section class='sys-main sys-dec-class dec-section-edit' :style="{ zoom: zoom }">
    ...
    <div class='dec-container-div'>
      <el-container>
        <el-container>
          <el-main style="padding:3px 5px 20px 0px;">
            <!---表头开始  -->
            <dec-head ref='decHead' :moduleName="moduleName" @drawArea='drawArea' ></dec-head>
            <!---表头结束  -->
            <!---表体开始  -->
            <dec-list ref='decList' @backStatisticsData='backStatisticsData' @drawBoyArea='drawBoyArea' :moduleName="moduleName"></dec-list>
          </el-main>
        </el-container>
      ...
      <!-- 引用识别记录 弹窗 end -->
        <div class='unDialog' :style="{display: unDialogVisible}" v-dialogMove>
        <!-- 标题栏 -->
        <div class='unDialog-header' ref="unDialog-header">
          <span style='font-size: 18px;'>识别对比</span>
          <span class='unDialog-close' @click="closeOcrCompare"><i class="fa fa-times" aria-hidden="true"></i></span>
        </div>
        <!-- 内容区 -->
        <div class='unDialog-tabs'>
          <el-tabs  v-model="activeName" @tab-click="handleClick">
            <el-tab-pane :label="item.fileName" v-for="(item, index) in picList" :name="item.fileName + '--' + item.picId" :key="'tab-' + index">
            </el-tab-pane>
          </el-tabs>
        </div>
        <div class="unDialog-container" style='width: 100%; height: 514px;overflow: hidden;'>
          <div class="toolbar">
            <el-button size="middle" type="default" icon='el-icon-zoom-in'  @click="addSize" ></el-button>
            <el-button size="middle" type="default" icon='el-icon-zoom-out' @click="subSize"></el-button>
          </div>
          <div style='width: 100%; height: 100%;' :style="{ transform: 'scale(' + zoom2 + ')'}" @mousedown="moveImg($event)">
            <div class='unDialog-content'>
              <img :src='currentPicInfo.url' style='width: 100%; height: 100%' ref="starryImg">
              <div :style='{top: top + "px", left: left + "px", width: width + "px", height: height + "px", display: signDiv}' class='unDialog-sign'></div>
            </div>
          </div>
        </div>
      </div>
...
import decHead from './decHead/decHead'
import decList from './decList/decList'
import decContainer from './container/container'
import decDocuments from './documents/documents'
import accompanyingDocuments from './components/accompanyingDocuments'
import decPageStore from './decPageStore'
// import {dataResult} from './common/test'
// import './common/dialogMove'
...
export default {
    ...
    // 指令的定义
      inserted: (el, binding, vnode, oldVnode) => {
        ...
        $header.onmousedown = function (s) {
          ...
          $header.setAttribute('moved', true)
          // 计算可移动的最大距离
          let maxX = $modal.offsetWidth - $dialog_.offsetWidth
  ...
  created () {
    ...
    this.moduleName = 'intelligentVouching' + params.funFlag + '-' + params.iEFlag + '-' + params.operationType + '-' + params.pid
    ...
    this.controller.pid = this.$route.params.operationType !== 'add' ? this.$route.params.pid : ''
    this.controller.isDisabled = this.$route.params.operationType === 'look'
  ...
  methods: {
    ...
    closeOcrCompare () {
      this.signDiv = 'none'
      this.unDialogVisible = 'none'
    },
    handleClick (data) {
      this.signDiv = 'none'
      this.activeName = data.name
      let num = data.name.split('--')[1]
      this.currentPicInfo = this.picList[num]
    },
    addSize () {
      this.zoom2 = this.zoom2 + 0.1
    },
    subSize () {
      if (this.zoom2 > 0) {
        this.zoom2 = this.zoom2 - 0.1
    ...
    drawArea (data) {
      if (this.picList && this.picList.length) {
        ...
        this.unDialogVisible = 'block'
        // 计算缩放比列
        let pic = this.picList[field.picId + '']
        let percentW = decUtil.Div(598, pic.width)
        let percentH = decUtil.Div(514, pic.height)
        let top = parseInt(decUtil.Mul(percentH, field.y))
        let left = parseInt(decUtil.Mul(percentW, field.x))
        let width = parseInt(decUtil.Mul(percentW, field.w))
        let height = parseInt(decUtil.Mul(percentH, field.h))
        // 切换tab
        this.activeName = pic.fileName + '--' + pic.picId
        this.currentPicInfo = pic
        // 圈图
        this.top = top
        this.left = left
        this.width = width
    ...
    drawBoyArea (data) {
      if (this.picList && this.picList.length) {
        let orcLength = +data.row
        if (orcLength > this.orcListFomart.length) {
          this.signDiv = 'none'
          return
        }
        let bodyfields = this.orcListFomart[orcLength - 1]
        let bodyfield = bodyfields[data.value]
        if (!bodyfield) {
          this.signDiv = 'none'
          return
        }
        // 计算缩放比列
        let pic = this.picList[bodyfield.picId + '']
        let percentW = decUtil.Div(598, pic.width)
        let percentH = decUtil.Div(514, pic.height)
        let top = parseInt(decUtil.Mul(percentH, bodyfield.y))
        let left = parseInt(decUtil.Mul(percentW, bodyfield.x))
        let width = parseInt(decUtil.Mul(percentW, bodyfield.w))
        let height = parseInt(decUtil.Mul(percentH, bodyfield.h))
        // 切换tab
        this.activeName = pic.fileName + '--' + pic.picId
        this.currentPicInfo = pic
        // 圈图
    ...
    getDecDetail (pid) {
      this.$post({
        url: 'API@/saas-ai/dec-ocr/getDecOcrTaskResult',
        data: pid,
        success: (res) => {
          ...
          // 处理表头数据
          let fields = {}
          let headVo = util.simpleClone(this.$refs.decHead.decHead)
          for (let i in arr) {
            let field = arr[i]
            fields[field.fieldCode] = {
              value: field.fieldValue,
              x: field.picReferences[0].x,
              y: field.picReferences[0].y,
              w: field.picReferences[0].w,
              h: field.picReferences[0].h,
              picId: field.picReferences[0].picId
            }
          ...
          for (let x in orcList) {
            bodyVo = this.gitInitBody()
            orcBodyVo = orcList[x]
            orcBodyAfter = {}
            for (let y in orcBodyVo) {
              bodyfield = orcBodyVo[y]
              orcBodyAfter[bodyfield.fieldCode] = {
                value: bodyfield.fieldValue,
                x: bodyfield.picReferences[0].x,
                y: bodyfield.picReferences[0].y,
                w: bodyfield.picReferences[0].w,
                h: bodyfield.picReferences[0].h,
                picId: bodyfield.picReferences[0].picId
              }
              ...
              if (bodyfield.fieldCode === 'gNo' && !bodyfield.fieldValue) {
                // orcBodyAfter['gNo'] = {
                //   value: bodyfield.fieldValue,
                //   x: bodyfield.picRefs[0].x,
                //   y: bodyfield.picRefs[0].y,
                //   w: bodyfield.picRefs[0].w,
                //   h: bodyfield.picRefs[0].h,
                //   picId: bodyfield.picRefs[0].picId
                // }
                bodyVo['gNo'] = +x + 1
                // orcBodyAfter['contrItem'] = {
                //   value: bodyfield.fieldValue,
                //   x: bodyfield.picRefs[0].x,
                //   y: bodyfield.picRefs[0].y,
                //   w: bodyfield.picRefs[0].w,
            ...
            orcBodyList.push(util.simpleClone(orcBodyAfter))
            listVo.push(util.simpleClone(bodyVo))
          }
          this.orcListFomart = orcBodyList
          console.log(this.orcListFomart)
          console.log(listVo)
          this.$refs.decList.getBodyDetail(listVo)
          let pics = util.simpleClone(dataResult.picDatas)
          let picsList = {}
          for (let n in pics) {
            let pic = pics[n]
            if (pic.picType.name !== '其他') {
              picsList[pic.picId + ''] = {
                fileName: pic.picType.name,
                height: pic.height,
                url: pic.url,
                width: pic.width,
                picId: pic.picId,
              ...
              if (pic.angle) {
                picsList[pic.picId + ''].url = ''
                this.getBase64(pic.url, pic.width, pic.height, -pic.angle, (dataUrl) => {
                  picsList[pic.picId + ''].url = dataUrl
                })
              }
            }
          }
          ...
          if (this.picList && this.picList.length) {
            let firstKey = Object.keys(picsList)[0]
            let activeObj = this.picList[firstKey]
            this.activeName = activeObj.fileName + '--' + activeObj.picId
            this.currentPicInfo = activeObj
            this.unDialogVisible = 'block'
    ...
    moveImg (e) {
      ...
      e.preventDefault()
      e.stopPropagation()
      let disX = (e.pageX - odiv.getBoundingClientRect().x) / this.zoom2
      let disY = (e.pageY - odiv.getBoundingClientRect().y) / this.zoom2
      if (!this.mouseX || this.$refs['unDialog-header'].getAttribute('moved') === 'true') {
        this.$refs['unDialog-header'].setAttribute('moved', false)
        this.mouseX = this.$refs.starryImg.getBoundingClientRect().x
        this.mouseY = this.$refs.starryImg.getBoundingClientRect().y
      }
      document.onmousemove = (e) => {
        let left = ((e.pageX - this.mouseX) / this.zoom2 - disX) * this.zoom2
        let top = ((e.pageY - this.mouseY) / this.zoom2 - disY) * this.zoom2
        currentTarget.style.transform = `translate3d(${left}px, ${top}px, 0px) scale(${this.zoom2})`
      }
