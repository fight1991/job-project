    ...
    <div class="query-table-common">
      ...
      <el-table class='sys-table-table' :data="resultList" border highlight-current-row size="mini" @selection-change="selectVal" :height="tabHeight">
        ...
        <el-table-column label="操作" width="120" align="center" fixed="right">
          <template slot-scope="scope">
            <el-button size="mini" type="text" class="table-icon list-icon-edit" title="编辑" v-if="scope.row.rcptStatus !== 'REC' && scope.row.rcptStatus !== 'ACC' && scope.row.rcptStatus !== 'WMA' && scope.row.rcptStatus !== 'END'" @click="editDetail('edit',scope.row.iprtTmpNo)"><i></i></el-button>
            <el-button size="mini" type="text" class="table-icon list-icon-look" title="详情" @click="editDetail('detail',scope.row.iprtTmpNo)"><i></i></el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
